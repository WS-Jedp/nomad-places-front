...
export const SearchSpotsGeneralFilters: React.FC<
  SearchSpotsGeneralFiltersProps
> = ({ closeCallback, defaultFilter }) => {
  ...
  function handleSpotTypeFilterCurrentValue() {
    ...
    if (selectedSpotTypesFilter.length === MAXIMUN_SPOT_TYPES)
      return t("filters.labels.all");

    const firstSpotType = spotTypesFilter.find(
      (spotType) => spotType.id === selectedSpotTypesFilter[0]
    );
    if (!firstSpotType) return t("filters.labels.none");

    if (selectedSpotTypesFilter.length > 1)
      return `${firstSpotType?.title} +${selectedSpotTypesFilter.length - 1}`;
  ...
  function handlPeopleAmountFilterCurrentValue() {
    ...
    const peopleAmountOption = spotAmountPeopleFilter.find(
  ...
  function handleMindsetFilterCurrentValue() {
    ...
    const mindsetOption = spotMindsetFilter.find(
      (mindset) => mindset.id === selectedSpotMindsetFilter[0]
    );
    if (!mindsetOption) return t("filters.labels.none");
    if (selectedSpotMindsetFilter.length > 1)
      return `${t(`filters.mindsets.${mindsetOption.name.toLowerCase()}`)} +${
        selectedSpotMindsetFilter.length - 1
      }`;
    return t(`filters.mindsets.${mindsetOption.name.toLowerCase()}`);
  }
  ...
  function handleKnownForFilterCurrentValue() {
    ...
    const mindsetOption = spotKnownForFilter.find(
      (mindset) => mindset.id === selectedSpotKnownForFilter[0]
    );
    if (!mindsetOption) return t("filters.labels.none");
    if (selectedSpotKnownForFilter.length > 1)
      return `${t(`filters.mindsets.${mindsetOption.name.toLowerCase()}`)} +${
        selectedSpotKnownForFilter.length - 1
      }`;
    return t(`filters.mindsets.${mindsetOption.name.toLowerCase()}`);
  }
  ...
  function handleCommoditiesFilterCurrentValue() {
    ...
    const commodityOption = spotCommoditiesFilter.find(
      (commodity) => commodity.id === selectedSpotCommoditiesFilter[0]
    );
  ...
  function handleRulesFilterCurrentValue() {
    ...
    const ruleOption = spotRulesFilters.find(
      (rule) => rule.id === selectedSpotRulesFilter[0]
    );
    if (!ruleOption) return t("filters.labels.any");
    if (selectedSpotRulesFilter.length > 1)
      return `${t(`filters.rules.${ruleOption.rule}`)} +${
        selectedSpotRulesFilter.length - 1
      }`;
    return t(`filters.rules.${ruleOption.rule}`);
  }
    ...
    >
      ...
      <IonRow className="w-full h-full overflow-y-auto pb-9 flex flex-col flex-nowrap items-center justify-start px-3 py-9">
        ...
        <SimpleDropdown
          ...
          closeCallback={() => setCurrentFilter(GeneralFiltersEnum.none)}
          disabled={!canUseRealTimeFilters()}
        >
