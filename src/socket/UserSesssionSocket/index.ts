import {
  JoinPlaceSessionRequestDTO,
  LeavePlaceSessionRequestDTO,
  PlaceSessionActionMessageDTO,
  PlaceSessionActionUpdateRequestDTO,
  PlaceSessionUpdateDTO,
  PlaceSessionUpdateMultipleActionsRequestDTO,
  QuickSessionReviewRequestDTO,
} from "../../dto/session/sockets";
...
export class UserSessionSocket {
  ...
  async quickJoinSesssion() {
    await this.socket.emit("quick-join-place-session", {
      placeID: this.placeID,
    });
  }
   ...
   * =================================
   */
  async updateSesssion(payload: {
    actionType: PLACE_SESSION_ACTIONS_ENUM;
    sessionID: string;
    actionData: PlaceSessionActionDataPayload;
  }) {
  ...
  async shareRecentActivity(payload: {
    sessionID: string;
    url: string;
    ...
    const actionData = {
      ...
      recentActivity: {
        ...
        username: this.username,
        userID: this.userID,
        userPhotoURL: payload.userProfilePicture
      }
    };
    await this.socket.emit("place-session-share-recent-activity", actionData);
  }
  ...
  onSessionUpdated(
    ...
    this.socket.on("place-session-update", (message: string) => {
      const payload:
        | PlaceSessionAction[]
        | { error: { type: string; message: string } } = JSON.parse(message);
      callback(payload);
    });
  }

  async onSessionMessage(
    callback: (payload: PlaceSessionActionMessageDTO) => void
  ) {
  ...
  async onSharedRecentActivity(
    callback: (payload: {
      action: PlaceSessionAction;
      userGamification?: UserGamification;
    }) => void
  ) {
    this.socket.on("place-session-share-recent-activity", (message: string) => {
      const payload: {
        action: PlaceSessionAction;
        userGamification?: UserGamification;
      } = JSON.parse(message);
